...
# Allow overriding the DB_PATH at runtime/build time; default to analytics.db
ENV DB_PATH=analytics.db
...
RUN mkdir -p data_generator/output && \
    ...
    cd .. && \
    python -m ingestion.parse_logs --log-path data_generator/output/telemetry_logs.jsonl --employee-path data_generator/output/employees.csv --db-path "$DB_PATH" --chunk-size 500 || true
