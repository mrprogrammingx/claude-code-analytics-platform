...
def detect_role_col(df: pd.DataFrame):
    ...
    for c in ["role", "user_role", "job_role", "position", "user_practice", "practice", "user.practice"]:
        if c in df.columns:
            return c
    for c in df.columns:
...
def main():
    ...
    if not emp_df.empty and not events_df.empty:
        ...
        if 'user_email' in events_df.columns and email_col:
            ...
            merged = events_df.merge(emp_df, left_on='attr_user_email', right_on=email_col, how='left')
        else:
            merged = events_df
        ...
        if role_to_use and 'output_tokens' in merged.columns:
            ...
            by_role = merged.groupby(role_to_use)['output_tokens_num'].sum().dropna().sort_values(ascending=False)
    ...
    if 'user_email' in events_df.columns:
        ...
        st.table(top_users.reset_index().rename(columns={'index': 'user_email', 'user_email': 'events'}))
    else:
    ...
    if st.button("Run query"):
        try:
            ...
            if choice == "Most used models":
                ...
                db = get_db_path()
                with duckdb.connect(db) as c:
                    cols = [r[0] for r in c.execute("SELECT column_name FROM information_schema.columns WHERE table_name='telemetry_events' AND table_schema='main'").fetchall()]
                # candidate columns to look for (order of preference)
                candidates = ["model", "attr_model", "model_name", "attr_model_name", "model.name"]
    ...
    with st.expander("Custom SQL (run any query)", expanded=expanded_state):
        ...
        if st.button("Run custom SQL"):
                ...
                def is_safe_sql(sql_text: str) -> tuple[bool, str]:
                    ...
                    if ';' in sql_clean and not sql_clean.rstrip().endswith(';') and sql_clean.count(';') > 0:
                    ...
                    if not re.match(r"^\s*(/\*.*?\*/\s*)*(--.*?\n\s*)*SELECT\b", sql_clean, flags=re.I|re.S):
